...
class TestP2PClient(unittest.TestCase):
    ...
    def setUp(self):
        ...
        cmd_ground = "python3 p2p_client.py --buddy={} --port_dest={}\
                    --port_source={}".format(
            self.buddy_usr_2, self.port_user_1, self.port_user_2
        )
    ...
    def test_send_message(self):
        ...
        (data_send, server_status, server_reason) = p2p_client.send_message(
            self.msg_from_air, self.ip_address, self.port_user_2, self.buddy_usr_1
        )
        ...
        # Air with an int msg
        (data_send, server_status, server_reason) = p2p_client.send_message(
            123456789, self.ip_address, self.port_user_2, self.buddy_usr_1
        )
        data_send = json.loads(data_send)
        ...
        (data_send, server_status, server_reason) = p2p_client.send_message(
            "", self.ip_address, self.port_user_2, self.buddy_usr_1
        )
        data_send = json.loads(data_send)
        ...
        (data_send, server_status, server_reason) = p2p_client.send_message(
            self.msg_json_air, self.ip_address, self.port_user_2, self.buddy_usr_1
        )
        data_send = json.loads(data_send)
        dico_msg = {
            "username": self.buddy_usr_1,
            "text": {"username": self.buddy_usr_1, "text": self.msg_from_air},
        }
        ...
        (data_send, server_status, server_reason) = p2p_client.send_message(
            list_msg, self.ip_address, self.port_user_2, self.buddy_usr_1
        )
        data_send = json.loads(data_send)
