...
class TestUserSrv(unittest.TestCase):
    ...
    def test_register(self):
        ...
        response = requests.post(
            self.SrvUrl + "/register",
            json={"pwd": "aAaa#a9aa", "ip": "0.0.0.4", "key": key1},
        )
        ...
        response = requests.post(
            self.SrvUrl + "/register",
            json={"username": "", "pwd": "aAaa#a9aa", "ip": "0.0.0.4", "key": key1},
        )
        self.assertEqual(response.status_code, 455)
        # field ip empty
        response = requests.post(
            self.SrvUrl + "/register",
            json={"username": "Ekdc", "pwd": "aAaa#a9aa", "ip": "", "key": key1},
        )
        ...
        response = requests.post(
            self.SrvUrl + "/register",
            json={"username": "Dkd5", "pwd": "", "ip": "192.0.0.8:5820", "key": key1},
        )
    ...
    def test_get_ip_port(self):
        ...
        response = requests.get(self.SrvUrl + "/get_ip_port/Nani")

        msg_received = response.json()

        self.assertEqual(msg_received, {"username": "Nani", "ip": ip, "port": port})
        self.assertEqual(response.status_code, 200)
        # have the ip of a user that is not present in the BDD
        response = requests.get(self.SrvUrl + "/get_ip_port/kakarotto")
